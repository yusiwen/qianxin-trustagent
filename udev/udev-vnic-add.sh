...
function process_dnat() {
  ...
  if [[ "$target" =~ "$ip_regex" ]]; then
    ...
    if [ "DNS_ENABLED" = 1 ]; then
      local resolved_ip=$(dig "$target" +short)
  ...
  ip route add "$target_ip" dev "$1" table dnat
  echo "ip route del $target_ip dev $1 table dnat" >> "$DNAT_FILE"
  if [ "$source_ip" != "$resolved_ip" ]; then
...
if [ -n "$DNAT_RULES" ]; then
  ...
  for d in "${dnats[@]}"; do
    IFS='-' read -r -a ips <<< "${d}"
    process_dnat "${ipd[0]}" "${ips[1]}"
  done
fi

/usr/bin/logger "route table & iptables rules added for $1"